...
import com.varabyte.kobweb.cli.common.warn
...
fun handleRun(
    ...
    KobwebGradle(env, projectDir).use { kobwebGradle ->
        handleRun(
            originalEnv,
            env,
            siteLayout,
...
private fun handleRun(
    ...
    if (runInPlainMode) {
        ...
        if (gradleArgsStop.isNotEmpty()) {
            println("Warning: --gradlestop is ignored when running in non-interactive mode (which does not stop the server).")
        }
